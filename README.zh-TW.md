...
- **免費、可證偽的驗證方式。** `sensor.basil_soil_voltage`／土壤濕度本來就會持續記錄，所以這個檢查完全不用額外成本：接下來幾天觀察它的走勢。如果它還是像包箔之前一樣會跌破 `Water below`（22%），代表蒸發量足夠，不用改動任何東西。如果它卡在 30% 左右不再往下掉，代表打孔密度不夠——該做的是多打幾個孔，而不是把鋁箔拆掉。
  ...
  **第一筆讀值（2026-09-21）：** 09-20 00:00 淹灌之後，土壤電壓從 0.3018 V 穩定上升，到 09-21 10:00 為 0.3213 V——大約每小時 +0.0006 V，約 34 小時後才回到 0.322 V 的「淹灌後靜置」錨點。土壤確實在變乾，沒有卡住；但上面兩種結果都還沒出現。照這個速度，距離 `Water below`（約 0.412 V）大概還有 6～7 天。至於**為什麼**乾得這麼慢，目前沒有定論——這個盆子沒有不包鋁箔時的乾燥速率可以對照——所以這裡刻意不把它歸因於鋁箔。有一個可以預期的副作用：澆水後 OLED 會長時間顯示 `SOIL 100%`，因為比 0.322 V 更濕的讀值一律被夾在 100%。這不是感測器卡住，這個區間內的變化只看得到 `soil_voltage`。
...
**記憶體：證據說了什麼，又沒說什麼。** 這一節先前的版本寫著「記憶體已被排除為機制」。那句話超出了證據所能支撐的範圍，在此撤回——因為這個撤回本身比原本那個結論更有教育意義。

**站得住的部分：當機當下的總可用記憶體並不異常。** 00:16:20 那筆新鮮讀值是 **6816 B**，正落在裝置隨後連續運作 **12.8 小時**、毫無事故的 **6.2–6.9 kB** 區間之內。而在切掉 `captive_portal` 之前，連續 27 分鐘的例行取樣停在 **4.0–4.7 kB**、沒有下降趨勢，所以並沒有記憶體洩漏。總可用記憶體本身，不足以解釋這個故障。

**站不住的，是這一節原本寫的那句**——「它當機時還有 11.6 kB 連續區塊，所以不是記憶體配置失敗」。那句話建立在「把記錄裡的某一行當成同一瞬間的快照」之上。而它並不是：
...
由此推出的結論並不令人舒服，但必須講清楚：**當機那一刻的 `max_block` 是未知的。**依照開機時的型態推斷，它當時應該已經回到訂閱狀態下的 ~3176 B 附近，而不是 11.6 kB。Noise 交握需要一塊多 kB 的**連續**緩衝區，而 ~3 kB 正好就是那個配置開始會失敗的區間。所以記憶體配置失敗**並沒有被排除**。`reset_reason` 也分不開這兩者：一次回傳 null 又被解參考的配置失敗，落地時同樣是 `Exception`，跟一個邏輯錯誤沒有兩樣。

**所以：觸發條件已經確立，機制仍然開放。** 交握路徑上的程式錯誤，與記憶體配置失敗，兩者都還活著。要分開它們，需要在故障發生的那一刻取樣 `max_block`，而目前這支 watcher 做不到——它需要逐欄位的時間戳記，或是只在三個欄位落在同一個刻度時才輸出。
...
### 一次三小時的停擺，原因是接頭鬆脫——以及它順便量到的東西

2026-09-20，HC-SR04 停止讀值，而且只要接著它板子就開不了機。原因是**杜邦接頭鬆
脫**，13:46 重新插好之後，感測器立刻恢復。這段記錄之所以留著，是因為它產出的兩
樣東西，價值高過故障本身。

**韌體是靠時間線排除的，不是靠論證。** 第一直覺是最近改的韌體把它弄壞了。而
00:58 燒錄的那一版，**持續穩定讀出 7.82–7.85 cm，一路到 10:24**，也就是 9.5 小
時。它是在一段實體作業的過程中失去回應的，**10:46:02** 給出最後一次正常讀值，
之後就再也沒有輸出，而裝置本身仍然活著。硬體介入的前後是同一個 binary，所以那
些修改不可能是原因。跑在 Pi 上的記錄器用一次查詢就解決了這件事，否則這會是一個
下午的韌體二分搜尋。

**「從頭到尾只是接觸問題」這件事的證據乾淨得不尋常：**

```
10:46:02  最後一次正常   7.82039976119995 cm   95.1166687011719 %
13:46:47  重新插好之後   7.82039976119995 cm   95.1166687011719 %
```

相隔三小時，逐位元完全相同。水位沒有變動，感測器也沒有漂移。

**而這個「完全相同」，把一件原本只是推論的事變成了量測。** 下面那一節論證：水位
感測器讀到 NaN 時，每一道泵浦閘門的比較都為假，所以澆水不可能發生。那原本是推
論。三個小時內水位一位數都沒變，就是證據：那段期間確實沒有任何循環跑過。13:00
時是推論，13:46 時是量測。

停擺期間曾記錄過一個工作假說——模組本身以某種會拉載 TRIG 的方式故障了，這一個
故障就能同時解釋讀值失效與開機失敗。**該假說已撤回。** 它夠精簡，而且是錯的；
接頭本來就在同一份嫌疑名單上，而且是上面最便宜的一項。要注意這對開機那個症狀的
意義：它**並不是**由 TRIG 被拉載所解釋，仍然是原本那個 GPIO2 strapping 風險。
接頭鬆脫或許會讓那個風險更容易浮現，但這一點並未被證實，也不該寫得像已經證實。

過程中另外兩個嫌疑犯也被排除了。ECHO 的 1k/2k 分壓是 **SMD 0805 焊死**的，不是
飛線，所以「分壓線被扯掉」從來就不成立。另外，鋁箔**並沒有**碰到 HC-SR04 附近
的任何東西。
...
### 水位感測器死掉的那段期間，這台裝置是安全的，但靠的是意外，不是設計
...
**而與 NaN 的任何比較都為假**，所以這兩道閘門過不了：循環無法啟動，正在跑的也
無法繼續。感測器死掉的狀態下，泵浦根本不可能運轉——而三小時後那個逐位元相同的
水位讀值，證實了那段期間確實一次都沒跑過。

同一套語意也悄悄讓「水箱見底」警報失效，那是同一件事的鏡像：停擺期間完全沒有水
位保護，有的只是「什麼都不會發生」。安全，但安全的理由沒有任何人設計過。
...
下次不管為了什麼原因拆線，YAML 裡都已經記著一個值得順手做掉的改善：**把 TRIG
從 D4/GPIO2 移到 D0/GPIO16**，這樣序列埠燒錄就不必再先拔掉感測器。那個 strapping
風險是一個被接受的設計妥協。它**並不是**這次停擺的成因，本節也刻意不把這次故障
寫成「證明了這件事非做不可」。


這個裝置還沒有「完工」。以下把已經確定的事，和還沒確定的事分開列：

- **已確定——觸發條件：** 在已有連線的狀態下，第二個 API 客戶端發起 Noise 握手會讓裝置當機。23 小時內零次 Exception 重開，武裝第二個客戶端後三分鐘內三次，停掉它之後就沒有了。這是實證結果，無論機制最後是什麼都成立。上面那些操作面與程式碼修正都由它推導而來。
- **未解——機制。** 交握路徑上的程式錯誤與記憶體配置失敗兩者都還活著，因為故障當下的 `max_block` 是未知的（為什麼記錄下來的那個值不能當快照用，見穩定性那一節）。要分開它們，需要在故障當下取樣 `max_block`，而目前這支 watcher 在沒有逐欄位時間戳記的情況下做不到。
